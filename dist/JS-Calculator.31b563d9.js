...
buttonsArr.forEach((button)=>{
    button.addEventListener("click", (e)=>{
        ...
        if (operators.includes(buttonValue)) {
            ...
            if (operators.includes(lastCharacter)) // if it is, remove it
            displayValue = displayValue.slice(0, -1);
        ...
        if (buttonValue === ".") {
            ...
            if (lastOperator) {
                ...
                const lastNumberAfterOperator = displayValue.slice(lastOperatorIndex + 1);
                // if the last number already has a decimal, return
                if (lastNumberAfterOperator.includes(".")) return;
            // if the display value is empty or ends with an operator, add a zero before the decimal
            } else if (displayValue.includes(".")) return; // if the display value already has a decimal, do nothing
        }
        ...
        if (buttonValue === "\xd7") displayValue += "*"; // replace × with *
        else if (buttonValue === "\xf7") displayValue += "/"; // replace ÷ with /
        else displayValue += buttonValue; // append the button value
        // update the display with the new value
        if (displayValue === "") displayValue = "0.00"; // if display value is empty, set it to 0.00
        if (displayValue.length > 15) displayValue = displayValue.slice(0, 15); // limit the display value to 15 characters
        displayInput(displayValue);
    });
});

},{}]},["5j6Kf","a0t4e"], "a0t4e", "parcelRequire42b5", {})

//# sourceMappingURL=JS-Calculator.31b563d9.js.map
